...
const testDir = process.env.TEST_DIR // Set in Task instance, expect form like /test/a/b
const outputPath = `${projectDir}${testDir}/.testOutput`;
const runDir = findNearestPackage(path.dirname(outputPath));
const runCommand = process.env.RUN_COM; // Set in image

const testFile = process.env.TEST_FILE || ""; // This should come from the Task instance
console.log(`Test file: "${testFile}"`);
if (testFile.endsWith(".ts")) {
	execSync(`cd ${testDir} && tsc ${testFile}`);
}
...
let result1 = runAndLoad(runDir, runCommand);

console.log("Before run 2");
let result2 = runAndLoad(runDir, runCommand);
...
function runAndLoad(dir, cmd) {
    try {
        execSync(`cd ${dir} && ${cmd}`, {timeout: 50 * 1000});
        return loadOutput();
    } catch (e) {
...
function findNearestPackage(currentPath) {
    console.log("Checking path " + currentPath + " for package.json");
    if (fs.readdirSync(currentPath).includes("package.json")) {
        return currentPath;
    } else if (currentPath === "../main") {
        return currentPath;
    } else {
        return findNearestPackage(path.dirname(currentPath));
    }
}
